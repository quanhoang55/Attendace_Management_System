...
class AttendanceManager:
    ...
    def recordAttendance(self, classId: str, date: str, studentId: str, status: str):
        ...
        student = schoolClass.getStudentList().find(
            lambda s: s.getStudentId() == studentId
        )
        if student is None:
            raise ValueError(
                f"Student with ID {studentId} is not enrolled in class {classId}."
            )
    ...
    def saveData(self):
        """
        Persists all current class, student, schedule,
        session, and attendance data to disk files.
        """
        ...
        while curr_class_node is not None:
            ...
            while curr_student is not None:
                ...
                students_content += (
                    f"{sc.classId}|{st.getStudentId()}|{st.getFullName()}\n"
                )
            ...
            while curr_schedule is not None:
                ...
                schedules_content += (
                    f"{sc.classId}|{sch.dayOfWeek}|{sch.period}|{sch.room}\n"
                )
    ...
    def loadData(self):
        """
        Loads and rebuilds classes, students, schedules,
        sessions, and attendance records from files.
        """
