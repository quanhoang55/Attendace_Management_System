...
class SchoolClass:
    ...
    def addStudent(self, student: Student):
        ...
        """
        existing = self.studentList.find(
            lambda s: s.getStudentId() == student.getStudentId()
        )
        if existing is not None:
            raise ValueError(
                f"Student ID {student.getStudentId()} is already enrolled in class {self.classId}."
            )
    ...
    def addSchedule(self, schedule: Schedule):
        ...
        duplicate = self.scheduleList.find(
            lambda s: s.dayOfWeek == schedule.dayOfWeek
            and s.period == schedule.period
